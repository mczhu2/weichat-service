    ...
    <div className="space-y-4">
      <div className="rounded-2xl border border-border bg-zinc-50/50 p-4">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <div className="text-sm font-medium text-foreground">缁勫悎娑堟伅缂栨帓</div>
            <div className="mt-1 text-xs leading-5 text-muted-foreground">
              缁勫悎娑堟伅浼氭寜鍒楄〃椤哄簭渚濇鍙戦€侊紝姣忎竴椤圭殑褰曞叆鏂瑰紡涓庡崟绫诲瀷妯℃澘淇濇寔涓€鑷淬€?            </div>
          </div>
      ...
      {items.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-border bg-white px-4 py-10 text-center text-sm text-muted-foreground">
          灏氭湭娣诲姞缁勫悎椤癸紝璇峰厛閫夋嫨娑堟伅绫诲瀷鍐嶆柊澧炪€?        </div>
      ) : (
        items.map((item, index) => (
          <div key={item.id} className="rounded-[24px] border border-border bg-white p-5 shadow-sm">
            <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <div className="text-sm font-medium text-foreground">绗?{index + 1} 椤?/div>
                <div className="mt-1 text-xs text-muted-foreground">
                  鎸夊綋鍓嶉『搴忓彂閫侊紝鍙殢鏃跺垏鎹㈡秷鎭被鍨嬫垨璋冩暣椤哄簭銆?                </div>
              </div>
