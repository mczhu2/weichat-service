...
export const TARGET_TYPE_OPTIONS: Array<{ value: TargetType; label: string; helper: string }> = [
  { value: 1, label: "澶栭儴鑱旂郴浜?, helper: "鎸変紒寰处鍙?vid 闆嗗悎鏌ヨ鍦堜汉鐩爣锛屼笉閫夊垯鏌ュ叏閮? },
  { value: 2, label: "缇よ亰", helper: "鎸変紒寰处鍙?vid 闆嗗悎鏌ヨ鍦堢兢鐩爣锛屼笉閫夊垯鏌ュ叏閮? },
];
...
export const MESSAGE_TYPE_OPTIONS = [
  ...
  { value: 3, label: "璇煶" },
  { value: 4, label: "瑙嗛" },
  { value: 5, label: "閾炬帴鍗＄墖" },
  { value: 6, label: "灏忕▼搴? },
  { value: 7, label: "缁勫悎娑堟伅" },
];
...
const TEMPLATE_MEDIA_RULES: Record<number, TemplateMediaRule> = {
  1: {
    ...
    hint: "鏀寔 jpg銆乸ng銆亀ebp銆乬if锛屽彲鎵归噺涓婁紶銆?,
  ...
  2: {
    ...
    hint: "鏀寔 pdf銆乨oc銆乨ocx銆亁ls銆亁lsx銆乼xt銆亃ip锛屽彲鎵归噺涓婁紶銆?,
    mimeTypes: new Set([
      "application/pdf",
      "application/msword",
  ...
  3: {
    ...
    hint: "鏀寔 mp3銆亀av銆乵4a銆乤ac銆乤mr锛屽彲鎵归噺涓婁紶銆?,
  ...
  4: {
    ...
    hint: "鏀寔 mp4銆乵ov銆乤vi銆乵kv锛屽彲鎵归噺涓婁紶銆?,
...
function validateTemplateUploadFile(file: File, msgType: number) {
  ...
  return `${rule.label}绫诲瀷鏍￠獙澶辫触锛屽綋鍓嶆枃浠?${file.name} 涓嶅湪鍏佽鑼冨洿鍐呫€俙;
}
...
function validateTemplateCompositeContent(raw: string) {
  ...
  if (!trimmed) return "缁勫悎娑堟伅妯℃澘鍐呭涓嶈兘涓虹┖";

  let parsed: unknown;
  try {
    parsed = JSON.parse(trimmed);
  } catch {
    return "缁勫悎娑堟伅妯℃澘鍐呭蹇呴』鏄悎娉?JSON 鏁扮粍";
  }

  if (!Array.isArray(parsed) || parsed.length === 0) {
    return "缁勫悎娑堟伅妯℃澘鍐呭蹇呴』鏄潪绌烘暟缁?;
  }

  for (let index = 0; index < parsed.length; index += 1) {
    const item = parsed[index];
    if (!item || typeof item !== "object") {
      return `绗?${index + 1} 椤瑰繀椤绘槸瀵硅薄`;
    }
    const candidate = item as Record<string, unknown>;
    const msgType = Number(candidate.msgType);
    if (!Number.isInteger(msgType) || msgType < 0 || msgType > 6) {
      return `绗?${index + 1} 椤圭殑 msgType 浠呮敮鎸?0-6`;
    }
    const templateContent = typeof candidate.templateContent === "string" ? candidate.templateContent.trim() : "";
    if (!templateContent) {
      return `绗?${index + 1} 椤圭殑 templateContent 涓嶈兘涓虹┖`;
    }
  }
...
export function formatTemplateContentPreview(template: MessageTemplate) {
  ...
  if (msgType === 5) {
    ...
    return [link.title, link.content, link.url].filter(Boolean).join(" 路 ") || "閾炬帴鍗＄墖鏈厤缃唴瀹?;
  }
  if (msgType === 6) {
    const app = parseTemplateMiniAppContent(template.templateContent);
    return [app.title, app.appName, app.pagepath].filter(Boolean).join(" 路 ") || "灏忕▼搴忔ā鏉挎湭閰嶇疆鍐呭";
  }
  if (msgType === 7) {
    const items = parseTemplateCompositeContent(template.templateContent);
    if (items.length === 0) {
      return "缁勫悎娑堟伅妯℃澘鏈厤缃唴瀹?;
    }
    const labels = items.slice(0, 3).map((item) => getMessageTypeLabel(item.msgType));
    const suffix = items.length > 3 ? ` 绛?${items.length} 椤筦 : ` 鍏?${items.length} 椤筦;
    return `缁勫悎娑堟伅锛?{labels.join("銆?)}${suffix}`;
  }
  if (!isMediaTemplateMsgType(msgType)) {
    return template.templateContent || "鏆傛棤鍐呭";
  }
  const parsed = parseTemplateMediaContent(template.templateContent);
  const summary = `${getMessageTypeLabel(msgType)}绱犳潗 ${parsed.items.length} 涓猔;
  if (parsed.text.trim()) {
    return `${parsed.text.trim()} 路 ${summary}`;
  }
  if (parsed.items.length > 0) {
    return `${summary} 路 ${parsed.items.map((item) => item.filename).slice(0, 2).join("銆?)}`;
  }
  return `${getMessageTypeLabel(msgType)}妯℃澘鏈厤缃礌鏉恅;
}
...
async function uploadFileBySignature(signature: MassUploadSignatureResult, file: File) {
  ...
  if (!response.ok) {
    throw new Error(`OSS 涓婁紶澶辫触锛圚TTP ${response.status}锛塦);
  }
}
...
async function uploadImageToOss(file: File) {
  ...
  if (!signatureRes.ok || !signatureRes.data) {
    throw new Error(signatureRes.error ?? "鑾峰彇鍥剧墖涓婁紶绛惧悕澶辫触");
  }
...
export function getMessageTypeLabel(msgType?: number | null) {
  return MESSAGE_TYPE_OPTIONS.find((item) => item.value === msgType)?.label ?? `绫诲瀷 ${msgType ?? "--"}`;
}
...
export function getTaskTypeLabel(taskType?: number | null) {
  if (taskType === 1) return "澶栭儴鑱旂郴浜?;
  if (taskType === 2) return "缇よ亰";
  return "--";
}
...
function getAccountFilterOptions(accounts: WecomAccount[]) {
  ...
  return accounts
    .map((account) => {
      ...
      return {
        value: vid,
        label: `${account.nickname || account.wxName || account.agentNo || "鏈懡鍚嶈处鍙?} 路 ${vid}`,
      };
    })
...
function FileUploadInput({
  ...
  return (
    <div className="space-y-2">
      ...
      >
        {loading ? (
          <span className="text-sm text-muted-foreground">澶勭悊涓?..</span>
        ) : preview ? (
          isImage ? (
            <img src={preview} alt="棰勮" className="max-h-32 max-w-full rounded-lg object-contain" />
          ) : (
            <span className="text-sm text-muted-foreground">鏂囦欢宸查€夋嫨</span>
          )
        ) : (
...
export function TaskDetailDialog({
  ...
  useEffect(() => {
    ...
    Promise.all([massTaskApi.getTask(taskId), massTaskApi.getTaskDetails(taskId)])
      .then(([taskRes, detailRes]) => {
        if (!active) return;
        if (!taskRes.ok) {
          setError(taskRes.error ?? "鍔犺浇浠诲姟澶辫触");
          return;
        }
        setTask(taskRes.data ?? null);
        setDetails(detailRes.ok ? detailRes.data ?? [] : []);
        if (!detailRes.ok) {
          setError(detailRes.error ?? "鍔犺浇鏄庣粏澶辫触");
        }
      })
      .catch((err) => {
        if (!active) return;
        setError(err instanceof Error ? err.message : "鍔犺浇浠诲姟澶辫触");
      })
      .finally(() => {
  ...
  return (
    <Dialog open={!!taskId} onClose={onClose} className="max-w-5xl rounded-[28px] border border-border/70">
      <DialogHeader>
        <DialogTitle>浠诲姟璇︽儏</DialogTitle>
      </DialogHeader>
      <DialogBody className="space-y-6">
        {loading ? (
          <div className="py-12 text-center text-sm text-muted-foreground">鍔犺浇涓?..</div>
        ) : error ? (
          ...
          <>
            <div className="grid gap-4 md:grid-cols-4">
              <div className="rounded-2xl border border-border bg-white px-4 py-4">
                <div className="text-xs uppercase tracking-[0.18em] text-muted-foreground">浠诲姟鍚嶇О</div>
                <div className="mt-2 text-sm font-medium text-foreground">{task.taskName || "--"}</div>
              </div>
              <div className="rounded-2xl border border-border bg-white px-4 py-4">
                <div className="text-xs uppercase tracking-[0.18em] text-muted-foreground">鍙戦€佺姸鎬?/div>
                <div className="mt-2 text-sm font-medium text-foreground">{TASK_STATUS_LABELS[task.sendStatus ?? 0] ?? "--"}</div>
              </div>
              <div className="rounded-2xl border border-border bg-white px-4 py-4">
                <div className="text-xs uppercase tracking-[0.18em] text-muted-foreground">鍙戦€佽繘搴?/div>
                <div className="mt-2 text-sm font-medium text-foreground">
                  {task.successCount ?? 0} / {task.totalCount ?? 0}
                </div>
              </div>
              <div className="rounded-2xl border border-border bg-white px-4 py-4">
                <div className="text-xs uppercase tracking-[0.18em] text-muted-foreground">璁″垝鏃堕棿</div>
            ...
            <div className="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
              <div className="rounded-2xl border border-border bg-white">
                <div className="border-b border-border px-5 py-4">
                  <h3 className="text-sm font-semibold text-foreground">鎺ユ敹鏄庣粏</h3>
                </div>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-border bg-zinc-50/70 text-left text-xs uppercase tracking-[0.14em] text-muted-foreground">
                        <th className="px-4 py-3">瀵硅薄</th>
                        <th className="px-4 py-3">鐘舵€?/th>
                        <th className="px-4 py-3">鍙戦€佹椂闂?/th>
                    ...
                    <tbody>
                      {details.length === 0 ? (
                        <tr>
                          <td colSpan={4} className="px-4 py-8 text-center text-muted-foreground">鏆傛棤鏄庣粏</td>
                        </tr>
                      ) : (
                        details.map((detail) => (
                          <tr key={detail.id} className="border-b border-border/70 last:border-b-0">
                            ...
                            <td className="px-4 py-3">
                              <StatusBadge
                                label={detail.isSent ? (DETAIL_STATUS_LABELS[detail.sendStatus ?? 0] ?? "宸插彂閫?) : "鏈彂閫?}
              ...
              <div className="rounded-2xl border border-border bg-white p-5">
                <h3 className="text-sm font-semibold text-foreground">浠诲姟鎽樿</h3>
                <div className="mt-4 space-y-3 text-sm">
                  <div className="flex items-center justify-between gap-4">
                    <span className="text-muted-foreground">鐩爣绫诲瀷</span>
                    <span className="text-foreground">{getTaskTypeLabel(task.taskType)}</span>
                  </div>
                  <div className="flex items-center justify-between gap-4">
                    <span className="text-muted-foreground">娑堟伅绫诲瀷</span>
                    <span className="text-foreground">{getMessageTypeLabel(task.msgType)}</span>
                  </div>
                  <div className="flex items-center justify-between gap-4">
                    <span className="text-muted-foreground">鍒涘缓浜?/span>
                    <span className="text-foreground">{task.creator || "--"}</span>
                  </div>
                  <div className="flex items-center justify-between gap-4">
                    <span className="text-muted-foreground">妯℃澘 ID</span>
                    <span className="text-foreground">{task.templateId ?? "--"}</span>
                  </div>
                  <div className="flex items-center justify-between gap-4">
                    <span className="text-muted-foreground">宸插彂閫?/span>
                    <span className="text-foreground">{task.sentCount ?? 0}</span>
                  </div>
                  <div className="flex items-center justify-between gap-4">
                    <span className="text-muted-foreground">鎴愬姛鏁?/span>
                ...
                <div className="mt-5 rounded-2xl bg-zinc-50 px-4 py-4 text-sm text-muted-foreground">
                  <div className="font-medium text-foreground">浠诲姟鍐呭</div>
                  <div className="mt-2 whitespace-pre-wrap break-words">{task.content || task.remark || "鏆傛棤鏂囨湰鍐呭"}</div>
                </div>
              </div>
            </div>
          </>
      ...
      <DialogFooter>
        <Button variant="secondary" onClick={onClose}>鍏抽棴</Button>
      </DialogFooter>
    </Dialog>
  );
}
...
function validateTemplateEditorState(
  ...
  if (msgType === 5) {
    if (!state.link.url.trim() || !state.link.title.trim() || !state.link.content.trim() || !state.link.imgurl.trim()) {
      return "閾炬帴鍗＄墖蹇呴』瀹屾暣濉啓缃戠珯鍦板潃銆佺綉绔欏悕绉般€佹弿杩板唴瀹瑰拰鍗＄墖灏侀潰";
    }
    return null;
  }
  if (msgType === 6) {
...
function ensureTaskMessageTypeSpecs(specs: MassMessageTypeSpec[]) {
  ...
  return [
    ...specs,
    {
      ...
      requiredFields: [],
      optionalFields: [],
      notes: ["缁勫悎娑堟伅鎵嬪姩鍐呭涓庢ā鏉垮唴瀹瑰潎鎸?template_content 缁撴瀯鐩存帴鍐欏叆 content銆?],
    },
...
export function TemplateDialog({
  ...
  async function uploadTemplateMediaFiles(msgType: number, files: File[]) {
    ...
    for (const file of files) {
      ...
      if (!signatureRes.ok || !signatureRes.data) {
        throw new Error(signatureRes.error ?? "鑾峰彇涓婁紶绛惧悕澶辫触");
      }
      await uploadFileBySignature(signatureRes.data, file);
      uploadedItems.push({
  ...
  async function handleSubmit() {
    ...
    if (msgType === 5) {
      ...
      if (!link.url.trim() || !link.title.trim() || !link.content.trim() || !link.imgurl.trim()) {
        setFormError("閾炬帴鍗＄墖蹇呴』瀹屾暣濉啓缃戠珯鍦板潃銆佺綉绔欏悕绉般€佹弿杩板唴瀹瑰拰鍗＄墖灏侀潰");
        return;
      }
      setFormError(null);
      await onSubmit({
    ...
    if (msgType === 6) {
      ...
      if (
        ...
        setFormError("灏忕▼搴忔ā鏉垮繀椤诲畬鏁村～鍐欏唴瀹广€佸悕绉般€佹爣棰樸€佸ご鍍忋€佸皝闈㈠浘銆佽烦杞湴鍧€銆佸師濮?ID 鍜?AppID");
        return;
      }
      setFormError(null);
      await onSubmit({
    ...
    if (supportsNameVariable && form.useNameVariable && !hasNamePlaceholder(content)) {
      setFormError("宸查€夋嫨鍙橀噺 name锛屾ā鏉垮唴瀹瑰繀椤诲寘鍚畬鏁寸殑 {{name}} 鍗犱綅绗?);
      return;
    }
    if (supportsNameVariable && !form.useNameVariable && hasNamePlaceholder(content)) {
      setFormError("鏈€夋嫨鍙橀噺 name 鏃讹紝妯℃澘鍐呭涓笉鑳藉寘鍚?{{name}}");
      return;
    }
    if (isMediaTemplateMsgType(msgType) && form.mediaItems.length === 0) {
      setFormError("褰撳墠娑堟伅绫诲瀷鑷冲皯闇€瑕佷笂浼?1 涓礌鏉?);
      return;
    }
  ...
  async function handleSelectMediaFiles(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setFormError(error instanceof Error ? error.message : "绱犳潗涓婁紶澶辫触");
    } finally {
      setUploading(false);
    }
  }
  ...
  async function handleUploadMiniAppCover(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setFormError(error instanceof Error ? error.message : "灏侀潰涓婁紶澶辫触");
    } finally {
      setUploading(false);
    }
  }
  ...
  return (
    <Dialog open={open} onClose={onClose} className="max-w-2xl rounded-[28px] border border-border/70">
      <DialogHeader>
        <DialogTitle>{template ? "缂栬緫妯℃澘" : "鏂板缓妯℃澘"}</DialogTitle>
      </DialogHeader>
      <DialogBody className="space-y-4">
        <div className="grid gap-4 md:grid-cols-2">
          <div>
            <label className="mb-1.5 block text-sm font-medium text-foreground">妯℃澘鍚嶇О</label>
            <Input
              value={form.templateName}
              onChange={(event) => setForm((prev) => ({ ...prev, templateName: event.target.value }))}
              placeholder="渚嬪锛氬叆缇ゆ椿鍔ㄩ€氱煡"
            />
          </div>
          <div>
            <label className="mb-1.5 block text-sm font-medium text-foreground">娑堟伅绫诲瀷</label>
            <Select
              value={form.msgType}
              onChange={(event) => {
        ...
        <div className={`grid gap-4 ${supportsNameVariable ? "md:grid-cols-2" : "md:grid-cols-1"}`}>
          {supportsNameVariable && (
            <div>
              <label className="mb-1.5 block text-sm font-medium text-foreground">鍙橀噺瀹氫箟</label>
              <div className="rounded-2xl border border-border bg-zinc-50/50 p-4">
                <label className="flex cursor-pointer items-start gap-3">
                  ...
                  <div className="space-y-1">
                    <div className="text-sm font-medium text-foreground">浣跨敤鍙橀噺 {TEMPLATE_VARIABLE_OPTIONS[0].label}</div>
                    <div className="text-xs leading-5 text-muted-foreground">
                      鍙橀噺瀹氫箟鍥哄畾涓烘灇涓惧€?`name`銆傚嬀閫夊悗妯℃澘鍐呭蹇呴』鍖呭惈瀹屾暣鐨?<code>{"{{name}}"}</code>锛涗笉鍕鹃€夋椂妯℃澘鍐呭涓嶈兘鍑虹幇瀹冦€?                    </div>
                  </div>
                </label>
              </div>
            </div>
          ...
          <div>
            <label className="mb-1.5 block text-sm font-medium text-foreground">鍒涘缓浜?/label>
      ...
      <DialogFooter>
        <Button variant="secondary" onClick={onClose} disabled={saving || uploading}>鍙栨秷</Button>
        <Button
          onClick={() => void handleSubmit()}
          disabled={saving || uploading || !form.templateName.trim()}
        >
          {saving ? "鎻愪氦涓?.." : uploading ? "涓婁紶涓?.." : template ? "淇濆瓨淇敼" : "鍒涘缓妯℃澘"}
        </Button>
      </DialogFooter>
    </Dialog>
  );
...
export function TaskCreateDialog({
  ...
  async function fetchTargets(pageNum: number, append: boolean) {
    ...
    try {
      ...
      if (!result.ok) throw new Error(result.error ?? "鏌ヨ鐩爣澶辫触");
      ...
      setTargetError(error instanceof Error ? error.message : "鏌ヨ鐩爣澶辫触");
      if (!append) {
        setTargets([]);
        setTargetPageNum(0);
        setTargetHasMore(false);
  ...
  async function handleSelectTaskManualMediaFiles(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setTaskFeedback(error instanceof Error ? error.message : "绱犳潗涓婁紶澶辫触");
    } finally {
      setTaskContentUploading(false);
    }
  }
  ...
  async function handleUploadTaskManualCover(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setTaskFeedback(error instanceof Error ? error.message : "灏侀潰涓婁紶澶辫触");
    } finally {
      setTaskContentUploading(false);
    }
  }
  ...
  async function handlePrecheck() {
    ...
    try {
      ...
      if (!result.ok) {
        setPrecheckErrors([result.error ?? "棰勬澶辫触"]);
        return false;
      }
  ...
  async function handleGenerateCopy() {
    ...
    const prompt = [
      "浣犳槸鎷涜仒杩愯惀缇ゅ彂鏂囨鍔╂墜銆傝鐢熸垚 3 鏉″彲鐩存帴鍙戦€佺殑涓枃鏂囨銆?,
      `娑堟伅绫诲瀷锛?{msgLabel}`,
      `闇€姹傛弿杩帮細${llmPrompt}`,
      "杈撳嚭瑕佹眰锛氫粎杈撳嚭 JSON 鏁扮粍瀛楃涓诧紝渚嬪 [\\\"鏂囨1\\\",\\\"鏂囨2\\\",\\\"鏂囨3\\\"]锛屼笉瑕侀澶栬鏄庛€?,
    ].join("\n");
    ...
    if (!result.ok || !result.data?.reply) {
      setLlmError(result.error ?? "鏂囨鐢熸垚澶辫触");
      return;
    }
  ...
  async function handleCreateTask() {
    if (selectedTargetIds.length === 0) {
      setTaskFeedback("璇峰厛閫夋嫨鑷冲皯涓€涓彂閫佺洰鏍?);
      setStep(1);
      return;
    }
    const valid = skipsPrecheck ? true : await handlePrecheck();
    if (!valid) {
      setTaskFeedback("棰勬鏈€氳繃锛岃鍏堜慨姝ｅ悗鍐嶅垱寤?);
      setStep(4);
      return;
    }
    ...
    if (!result.ok) {
      setTaskFeedback(result.error ?? "鍒涘缓浠诲姟澶辫触");
      return;
    }
    ...
    if (autoTrigger && result.data) {
      ...
      if (!triggerRes.ok) {
        setTaskFeedback(`浠诲姟宸插垱寤猴紙ID: ${result.data}锛夛紝浣嗚Е鍙戝け璐ワ細${triggerRes.error ?? "鏈煡閿欒"}`);
      }
    }
      ...
      return (
        <div key={field.field}>
          ...
          <FileUploadInput
            ...
            accept={field.label?.includes("鍥剧墖") ? "image/*" : field.label?.includes("璇煶") ? "audio/*" : "*/*"}
            placeholder={field.description || "鐐瑰嚮閫夋嫨鏂囦欢锛屾垨鎷栨嫿鏂囦欢鍒版澶?}
          />
        </div>
      );
    }
  ...
  return (
    <Dialog open={open} onClose={handleDialogClose} className="max-w-6xl rounded-[28px] border border-border/70">
      <DialogHeader>
        <DialogTitle>鍒涘缓缇ゅ彂浠诲姟</DialogTitle>
      </DialogHeader>
      ...
      <DialogBody className="space-y-5">
        ...
        {step === 1 && (
          <section className="rounded-[24px] border border-border bg-zinc-50/40 p-5">
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                <label className="mb-1.5 block text-sm font-medium text-foreground">鐩爣绫诲瀷</label>
              ...
              <div>
                <label className="mb-1.5 block text-sm font-medium text-foreground">鍙戦€佽处鍙风瓫閫夛紙鍙笉閫夛級</label>
                ...
                <p className="mt-1 text-xs text-muted-foreground">涓嶉€夋嫨璐﹀彿鏃讹紝浼氭煡璇㈣鐩爣绫诲瀷涓嬬殑鍏ㄩ儴鍙敤瀵硅薄銆?/p>
              </div>
            </div>
            ...
            <div className="mt-4 flex items-center justify-between text-sm text-muted-foreground">
              <span>宸查€夋嫨 <span className="font-semibold text-foreground">{selectedTargetIds.length}</span> 涓洰鏍?/span>
              <div className="flex items-center gap-3">
                <button type="button" className="text-xs underline underline-offset-4" onClick={() => setSelectedTargetIds([])}>娓呯┖</button>
                <button type="button" className="text-xs underline underline-offset-4" onClick={() => setSelectedTargetIds(targets.map((item) => Number(item.id)))}>鍏ㄩ€夊綋鍓嶇粨鏋?/button>
              </div>
            </div>
            ...
            >
              {targets.length === 0 ? (
                <div className="px-6 py-12 text-center text-sm text-muted-foreground">{targetsLoading ? "鑷姩鏌ヨ涓?.." : "鏆傛棤鐩爣锛岃璋冩暣绛涢€夋潯浠?}</div>
              ) : (
                    ...
                    return (
                      <label key={`${targetType}-${id}`} className={`flex cursor-pointer items-start gap-4 px-5 py-4 ${checked ? "bg-zinc-50" : "hover:bg-zinc-50/70"}`}>
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="mt-1 truncate text-xs text-muted-foreground">{getTargetMeta(target, targetType) || "鏃犺ˉ鍏呬俊鎭?}</div>
                        </div>
                        <div className="text-xs text-muted-foreground">#{id}</div>
                      </label>
                    );
                  })}
                  {targetHasMore && <div className="px-5 py-3 text-center text-xs text-muted-foreground">{targetsLoading ? "鍔犺浇鏇村涓?.." : "婊氬姩鍒板簳閮ㄨ嚜鍔ㄥ姞杞?}</div>}
                </div>
              )}
            </div>
          </section>
        ...
        {step === 2 && (
          <section className="rounded-[24px] border border-border bg-white p-5">
            <div className="flex items-center gap-3">
              <button type="button" onClick={() => setContentMode("manual")} className={`rounded-full border px-3 py-1 text-xs ${contentMode === "manual" ? "border-black bg-black text-white" : "border-border text-muted-foreground"}`}>鎵嬪姩鍐呭锛堥粯璁わ級</button>
              <button type="button" onClick={() => setContentMode("template")} className={`rounded-full border px-3 py-1 text-xs ${contentMode === "template" ? "border-black bg-black text-white" : "border-border text-muted-foreground"}`}>浣跨敤妯℃澘</button>
            </div>
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-2">
              ...
              {contentMode === "template" && (
                <div>
                  <label className="mb-1.5 block text-sm font-medium text-foreground">妯℃澘</label>
                  <Select className="w-full" value={taskForm.templateId} onChange={(event) => setTaskForm((prev) => ({ ...prev, templateId: event.target.value }))}>
                    <option value="">璇烽€夋嫨妯℃澘</option>
                    {templates.map((template) => (
                      <option key={template.id} value={template.id}>{template.templateName || `妯℃澘 ${template.id}`}</option>
                    ))}
                  </Select>
                </div>
              )}
        ...
        {step === 3 && (
          <section className="rounded-[24px] border border-border bg-white p-5">
            <div className="grid gap-4 md:grid-cols-2">
              <div>
                <label className="mb-1.5 block text-sm font-medium text-foreground">浠诲姟鍚嶇О锛堝彲閫夛級</label>
                <Input value={taskForm.taskName} onChange={(event) => setTaskForm((prev) => ({ ...prev, taskName: event.target.value }))} placeholder="渚嬪锛氬懆涓€閭€绾︽壒娆? />
              </div>
              <div>
                <label className="mb-1.5 block text-sm font-medium text-foreground">鍒涘缓浜猴紙鍙€夛級</label>
                <Input value={taskForm.creator} onChange={(event) => setTaskForm((prev) => ({ ...prev, creator: event.target.value }))} placeholder="渚嬪锛氳繍钀ヤ竴缁? />
              </div>
              <div>
                <label className="mb-1.5 block text-sm font-medium text-foreground">鍙戦€佹椂闂达紙鍙€夛級</label>
                <Input type="datetime-local" value={taskForm.sendTime} onChange={(event) => setTaskForm((prev) => ({ ...prev, sendTime: event.target.value }))} />
                <p className="mt-1 text-xs text-muted-foreground">鐣欑┖榛樿涓虹珛鍗冲彂閫侊紙鎸夊悗绔瓥鐣ユ墽琛岋級銆?/p>
              </div>
              <div>
                <label className="mb-1.5 block text-sm font-medium text-foreground">澶囨敞锛堝彲閫夛級</label>
                <Input value={taskForm.remark} onChange={(event) => setTaskForm((prev) => ({ ...prev, remark: event.target.value }))} placeholder="缁欏崗浣滃悓瀛︾殑澶囨敞" />
              </div>
            </div>
        ...
        {step === 4 && (
          <section className="rounded-[24px] border border-border bg-white p-5">
            <div className="grid gap-3 text-sm md:grid-cols-2">
              <div className="rounded-xl bg-zinc-50 px-4 py-3">鐩爣绫诲瀷锛歿getTaskTypeLabel(targetType)}锛岀洰鏍囨暟锛?span className="font-semibold text-foreground">{selectedTargetIds.length}</span></div>
              <div className="rounded-xl bg-zinc-50 px-4 py-3">娑堟伅绫诲瀷锛歿getMessageTypeLabelFromSpecs(selectedMsgType, sortedSpecs)}</div>
              <div className="rounded-xl bg-zinc-50 px-4 py-3">鍐呭妯″紡锛歿contentMode === "manual" ? "鎵嬪姩鍐呭" : "妯℃澘"}</div>
              <div className="rounded-xl bg-zinc-50 px-4 py-3">鑷姩瑙﹀彂锛歿autoTrigger ? "鏄? : "鍚?}</div>
            </div>
            ...
            {skipsPrecheck ? (
              ...
              <Button variant="secondary" size="sm" onClick={() => void handlePrecheck()} disabled={prechecking}>{prechecking ? "棰勬涓?.." : "鎵ц棰勬"}</Button>
              <span className="text-xs text-muted-foreground">棰勬浼氳皟鐢ㄥ悗绔?validateTask锛屾彁鍓嶆毚闇插瓧娈甸棶棰樸€?/span>
              </div>
            )}

            {!skipsPrecheck && precheckErrors.length > 0 && (
              <div className="mt-4 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
                <div className="font-medium">棰勬鏈€氳繃</div>
      ...
      <DialogFooter className="items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <Button variant="secondary" onClick={handleDialogClose} disabled={taskSubmitting}>鍙栨秷</Button>
          {step > 1 && <Button variant="secondary" onClick={() => setStep((prev) => (prev - 1) as 1 | 2 | 3 | 4)} disabled={taskSubmitting}>涓婁竴姝?/Button>}
          {step < 4 && (
            <Button
              onClick={() => {
                if (step === 1 && !canMoveStep2) {
                  setTaskFeedback("璇疯嚦灏戦€夋嫨 1 涓彂閫佺洰鏍?);
                  return;
                }
          ...
          {step === 4 && (
            <Button onClick={() => void handleCreateTask()} disabled={taskSubmitting || prechecking}>{taskSubmitting ? "鍒涘缓涓?.." : "鍒涘缓浠诲姟"}</Button>
          )}
        </div>
      </DialogFooter>
    </Dialog>
