...
export function TaskCreateDialog({
  ...
  function handleDialogClose() {
    if (taskSubmitting || taskContentUploading) return;
    onClose();
  }
  ...
  return (
    <Dialog open={open} onClose={handleDialogClose} className="max-w-6xl rounded-[28px] border border-border/70">
      ...
      <DialogBody className="space-y-5">
        ...
        {step === 2 && (
          <section className="rounded-[24px] border border-border bg-white p-5">
            ...
            {contentMode === "manual" && (
              <>
                {taskContentUploading && (
                  <div className="mt-5 rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-muted-foreground">
                    素材上传中，请等待上传完成后再继续下一步或创建任务。
                  </div>
                )}
      ...
      <DialogFooter className="items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <Button variant="secondary" onClick={handleDialogClose} disabled={taskSubmitting || taskContentUploading}>取消</Button>
          {step > 1 && <Button variant="secondary" onClick={() => setStep((prev) => (prev - 1) as 1 | 2 | 3 | 4)} disabled={taskSubmitting || taskContentUploading}>上一步</Button>}
          {step < 4 && (
            <Button
              disabled={taskSubmitting || taskContentUploading}
              onClick={() => {
                if (taskContentUploading) {
                  setTaskFeedback("素材上传中，请等待上传完成");
                  return;
                }
          ...
          {step === 4 && (
            <Button onClick={() => void handleCreateTask()} disabled={taskSubmitting || prechecking || taskContentUploading}>{taskSubmitting ? "创建中..." : taskContentUploading ? "上传中..." : "创建任务"}</Button>
          )}
        </div>
      </DialogFooter>
    </Dialog>
